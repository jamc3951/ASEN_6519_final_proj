...
%BS Stuff ---- need to currect get bin 1-4, success >= z*
N = 50;
c = 0.1;
for i = 1:N
        i
       [goaest,outcomebin,current,adversary,goal] = phase3_but_a_function(simcurrp3, simadvp3, simoutcp3, c);
        ...
        for w = 1:length(goaest)
          ...
          regret = zeros(1,w);
          regret2 = zeros(1,w);
          forecast_prob(w) = outcomebin(4,w);
          result =  runMCSims(workspace1,current(w,:),goal,adversary(w,:),1,0.4,inf);
          [outcomes,bins,xBins] = composite_ordering(result,d_0);
