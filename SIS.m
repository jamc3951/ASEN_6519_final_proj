...
for j = 1:C_s
    ...
    current_kp(j,:) = c(2,:);
    adversary_kp(j,:) = a(2,:);
    char_weights(j)=mvnpdf([current_kp(j,:),adversary_kp(j,:)],ykp(:)',10*eye(4)); % I thinnk you have the order messed up on the ypk vs kp if that makes sense
...
%  
    if outcomes(c) == 1 %Variance?
        outcomes_kp(2) = outcomes_kp(2) + char_weights(c)*mvnpdf([current_c,adversary_c],ykp(:)',Py*eye(4)).*...
            mvnpdf([current_c,adversary_c],[current_kp(c,:),adversary_kp(c,:)],Ps*eye(4));
    else
        outcomes_kp(1) = outcomes_kp(1) + char_weights(c)*mvnpdf([current_c,adversary_c],ykp(:)',Py*eye(4)).*...
            mvnpdf([current_c,adversary_c],[current_kp(c,:),adversary_kp(c,:)],Ps*eye(4));
    end
end
t = sum(outcomes_kp);
